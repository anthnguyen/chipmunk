...
A \emph{linear probe} is a small classifier trained to recover a fact from the
model's internal activity. We compared two linear probes:
...
If both probes work, the representation stayed similar. If the new probe works but
the old probe does not, the information remains present but has moved into different
internal coordinates.

That is what we observed. At layer 26, each organism's own size probe was essentially
perfect (mean 99.6\%), while the original probe averaged 71.9\%.
